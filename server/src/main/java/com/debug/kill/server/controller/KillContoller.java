...
@Controller
public class KillContoller {
    private static final Logger Log = LoggerFactory.getLogger(KillContoller.class);
    ...
    public BaseResponse execute(@RequestBody @Validated KillDto dto, BindingResult result, HttpSession session) {
        ...
        try {
            Boolean res = killService.killItem(dto.getKillId(), 1);
            if (!res) {
                return new BaseResponse(StatusCode.Fail.getCode(), "商品已经抢购完了");
            }
        } catch (Exception e) {
